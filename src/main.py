...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # tighten later to your Vercel domains
...
# -------- stubs you can replace with real data later --------

async def handle_dashboard(input: Dict[str, Any], company_id: str) -> Dict[str, Any]:
    return {
        "kpis": [
...
async def handle_financial_overview(input: Dict[str, Any], company_id: str) -> Dict[str, Any]:
    dso_days = 36.0
    inventory_turns = 6.2
    dpo_days = 42.0
    days_inventory = 365.0 / inventory_turns if inventory_turns > 0 else 0.0
